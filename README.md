...
> Built by Diego Alonso del Río García — Backend Engineer | SRE | Lima, Peru

---

## What Is GEMA?

GEMA is a personal job-hunting automation system. It scrapes multiple remote job boards simultaneously using a headless Chromium browser, sends every job listing through a chain of LLMs for structured data extraction, scores each result against a personalized CV profile, and delivers real-time Discord notifications — all from a Streamlit UI you run locally.

It was built out of frustration. Manually scanning 10+ job boards daily, copy-pasting job descriptions, and filtering noise by hand is a full-time job in itself. GEMA automates the entire pipeline: scrape → extract → score → notify → export.
...
User Prompt (natural language)
   ...
   AI Auditor  (LLM #2 cross-checks the config)
        │
        ▼
   Playwright Scraper (14 domains, concurrent tabs)
        │
        ▼
...
- **Natural language search** — describe what you want in plain English, the LLM parses it into a validated `SearchConfig`
...
- **14 job board scrapers** — himalayas.app, trueup.io, remote.co, weworkremotely.com, remoteok.com, workingnomads.com, news.ycombinator.com, wellfound.com, arc.dev, builtin.com, welcometothejungle.com, remotivated.com, posthog.com, greenhouse.com
- **Anti-bot resilience** — Playwright stealth, User-Agent spoofing, per-domain circuit breakers, jitter between requests
- **Personalized scoring** — matches job tech stacks against a `user_profile.yaml` derived from your actual CV
...
1. Type a search in plain English — e.g. *"Backend Python engineer, remote, salary above 80k"*
...
4. You review the JSON, edit if needed, and hit **Confirm & Start Extraction**
...
7. The matcher scores every job against your CV profile
...
## The Honest Development Log — Where It Broke and How It Got Fixed
...
**The wrong fix:** Changed `from playwright_stealth import stealth_async` to `from playwright_stealth import stealth`. This also crashed — `stealth` in v2 is a *module*, not a callable.

**The real fix:** Inspected the installed package with `inspect.iscoroutinefunction()` and `dir()` to discover the new API. The correct v2 call is:
```python
...
**Root cause:** `load_dotenv()` with no arguments searches the *current working directory*. When Streamlit launches, the CWD is wherever you ran the command from — not necessarily the project folder.
...
**What happened:** Discord was receiving 2–3 "GEMA is starting" messages per run. Streamlit re-renders the entire script on every user interaction, so any code not guarded by session state runs multiple times.

**The fix:** Added a `start_webhook_sent` flag to `DEFAULTS` and guarded the alert:
```python
...
**What happened:** Cohere's fallback was throwing 404 errors. The first assumption was the endpoint URL (`.ai` vs `.com`). Several commits went back and forth on this.
...
**The fix:** Live-tested every candidate replacement model against the real API key before committing. `command-r-plus-08-2024` returned HTTP 200.
...
**What happened:** OpenRouter also threw 404s. The model string was `google/gemma-2-27b-it:free`. The assumption was the `:free` suffix was invalid.

**What actually happened:** OpenRouter returned HTTP 200 when tested directly with `google/gemma-2-27b-it` (without `:free`). The model without the suffix is the paid variant but worked with the account's credits. Live API test confirmed it before any code changed.
...
**The fix:** Capture the value on the main thread at spawn time and pass it via `args`:
```python
...
**The fix:** Raised `MAX_RATE_LIMIT_HITS` from 5 to 50 in both `config.py` and `.env`. Also injected a realistic Windows/Chrome User-Agent at browser context creation level:
```python
user_agent=(
...
**The fix:** After the LLM generates the config, immediately override `target_domains` with the full registry:
```python
...
**What's embarrassing about this:** It was caught by the user running the app, not by a test. A simple `python -c "import nlp_engine"` would have caught it immediately.
...
├── selectors_registry.py    # 14 domain scraping contracts
...
- **Login-gated boards** (wellfound, arc.dev, welcometothejungle) return zero results until credentials are added — handled gracefully but not scraped
- **Cloudflare-heavy boards** (remoteok.com) may still get blocked despite stealth mode
- **HN Jobs** doesn't support keyword filtering — returns all current postings regardless of title query
- **Selector drift** — any job board can change its HTML structure; when that happens, use the God Mode override in the sidebar to inject custom selectors without touching code
...
- [ ] Selector staleness alerts in sidebar (wired but not surfaced)
...
**Diego Alonso del Río García**  
Backend Engineer | Data Integrity | SRE | DevSecOps  
Lima, Peru — UTC-5

Built this because job hunting at scale deserved the same rigor I apply to production systems.
